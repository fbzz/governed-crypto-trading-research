"""Frozen V77 persistent multi-horizon duration training orchestration."""
...
from .persistent_duration_training_data import (
    V77FoldTrainingData,
    read_v77_fold_training_data,
)
from .persistent_duration_training_engine import (
...
class V77TrainingError(RuntimeError):
    pass
...
def _load_json(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise V77TrainingError(f"Unable to read {label}: {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise V77TrainingError(f"{label} must be an object")
    return value
...
def _git_receipt(root: Path, require_clean: bool) -> dict[str, Any]:
    ...
    if require_clean and status:
        raise V77TrainingError("V77 requires a clean committed Git source receipt")
...
def _process_lock(path: Path, operation: str) -> Iterator[None]:
    ...
    try:
        ...
        raise V77TrainingError(f"V77 optimizer lock is active: {path}") from exc
    try:
        os.write(
            descriptor,
...
def _context(config: dict[str, Any]) -> dict[str, Any]:
    training = config.get("persistent_duration_training")
    if not isinstance(training, dict) or training.get("version") != "v77":
        raise V77TrainingError("Missing frozen V77 training config")
    ...
    if current["phase_contract"] != {
        ...
        raise V77TrainingError("V77 live phase-contract reference drift")
    contract = yaml.safe_load(contract_path.read_text(encoding="utf-8"))
    if (
        contract.get("phase") != "v77"
        or contract.get("stage_revision")
        != "v077_frozen_non_target_persistent_duration_training_r3"
    ):
        raise V77TrainingError("V77 frozen phase revision drift")
    inputs = {
    ...
    if (
        ...
        raise V77TrainingError("V77 input allowlist or content hash drift")
    metadata = {
    ...
    if blueprint.get("candidate_family_id") != contract["family_id"]:
        raise V77TrainingError("V77 blueprint family drift")
    ...
    if not source_files or len(source_files) != len(set(source_files)):
        raise V77TrainingError("V77 source receipt list is empty or duplicated")
    source_hashes = {
        relative: file_sha256(root / relative) for relative in source_files
    }
    source_receipt = {
        "schema_version": "v77-training-source-receipt/v1",
        "git_clean": git["clean"],
        "git_head": git["head"],
        "files": source_hashes,
...
def _storage_policy_receipt(context: dict[str, Any]) -> dict[str, Any]:
    ...
    waiver = _load_json(waiver_path, "V77 owner storage waiver")
    if not (
        file_sha256(waiver_path) == waiver_ref["file_sha256"]
        and waiver.get("phase") == "v77"
        and waiver.get("risk_acceptance") is True
        and waiver.get("waived_safeguards") == policy["waived_safeguards"]
    ):
        raise V77TrainingError("V77 owner storage waiver failed verification")
    value = {
        "version": "v77_backup_policy_receipt_v1",
        "phase": "v77",
        "mode": "owner_waiver",
        "verified": True,
...
def _doctor_or_raise(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if doctor.get("full_training_ready") is not True:
        raise V77TrainingError(
            "V77 runtime doctor blocks execution: "
            + json.dumps(
                {
                    key: doctor.get(key)
...
def _job_context(
    context: dict[str, Any], data: V77FoldTrainingData, fold: int, seed: int
) -> dict[str, Any]:
    return {
        "phase": "v77",
...
def _preflight(context: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "parameter_count_exact": parameter_count == 1_083_155,
        ...
        == "sealed",
        "v76_dataset_audit_passed": context["metadata"]["v76_audit"]["passed"]
        is True,
    }
    if not all(checks.values()):
        raise V77TrainingError(f"V77 preflight failed: {checks}")
    phase_reference = {
        "path": _relative(context["root"], context["contract_path"]),
        "file_sha256": context["contract_hash"],
    }
    training_spec = {
        "schema_version": "v77-training-spec/v1",
    ...
    input_receipt = {
        "schema_version": "v77-input-hash-receipt/v1",
    ...
    result = {
        "schema_version": "v77-training-preflight/v1",
        "decision": "authorize_v77_interrupted_resume_mps_smoke_only",
...
def _require_prior(
    ...
    if (
        ...
        raise V77TrainingError(f"V77 prior gate failed or drifted: {name}")
    return value


def _fold_data(context: dict[str, Any], fold: int) -> V77FoldTrainingData:
    return read_v77_fold_training_data(
        root=context["root"],
...
def _smoke(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if path.is_file():
        existing = _load_json(path, "V77 smoke")
        if existing.get("audit", {}).get("passed") is True:
            return existing
    _require_prior(
        context,
        "preflight.json",
        "authorize_v77_interrupted_resume_mps_smoke_only",
    )
    ...
    if not all(checks.values()):
        raise V77TrainingError(f"V77 smoke failed: {checks}")
    result = {
        "schema_version": "v77-training-smoke/v1",
        "decision": "authorize_v77_full_nine_job_training_only",
        "audit": {"passed": True, "checks": checks},
...
def _checkpoint_manifest(
    ...
    value = {
        "schema_version": "v77-checkpoint-manifest/v1",
...
def _full(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if path.is_file():
        existing = _load_json(path, "V77 training result")
        if existing.get("audit", {}).get("passed") is True:
            return existing
    _require_prior(
        context, "smoke.json", "authorize_v77_full_nine_job_training_only"
    )
    grid = context["contract"]["grid_optimizer_and_runtime_contract"]
    jobs: list[dict[str, Any]] = []
    contexts: dict[str, dict[str, Any]] = {}
    ...
    if not all(checks.values()):
        raise V77TrainingError(f"V77 full training audit failed: {checks}")
    checkpoint_manifest = _checkpoint_manifest(context, jobs, contexts)
    grid_manifest = {
        "schema_version": "v77-grid-manifest/v1",
        "expected_jobs": expected,
    ...
    result = {
        "schema_version": "v77-training-result-intermediate/v1",
        "decision": "authorize_v77_checkpoint_verification_only",
        "audit": {"passed": True, "checks": checks},
        "jobs": jobs,
        "summary": {
            "completed_jobs": 9,
...
def _verify(context: dict[str, Any]) -> dict[str, Any]:
    ...
    if path.is_file():
        existing = _load_json(path, "V77 verification")
        if existing.get("audit", {}).get("passed") is True:
            return existing
    _require_prior(
        context,
        "training_result.json",
        "authorize_v77_checkpoint_verification_only",
    )
    manifest = _load_json(
        context["output"] / "checkpoint_manifest.json",
        "V77 checkpoint manifest",
    )
    rows = [
        verify_v77_checkpoint(
    ...
    if not all(checks.values()):
        raise V77TrainingError(f"V77 verification failed: {checks}")
    result = {
        "schema_version": "v77-training-verification/v1",
        "decision": "authorize_v77_zero_step_replay_only",
        "audit": {"passed": True, "checks": checks},
        "verification": {
...
def _replay(context: dict[str, Any]) -> dict[str, Any]:
    _require_prior(
        context, "verification.json", "authorize_v77_zero_step_replay_only"
    )
    manifest = _load_json(
        context["output"] / "checkpoint_manifest.json",
        "V77 checkpoint manifest",
    ...
    if not all(checks.values()):
        raise V77TrainingError(f"V77 zero-step replay failed: {checks}")
    replay = {
        "schema_version": "v77-training-replay/v1",
        "decision": "authorize_v77_training_gate_registration_only",
        "audit": {"passed": True, "checks": checks},
        "replay": {
            "new_jobs": 0,
...
def _operator_packet(context: dict[str, Any], operation: str) -> None:
    ...
    if result.returncode:
        raise V77TrainingError(
            f"V77 operator packet {operation} failed: {result.stderr.strip()}"
        )


def _finalize(context: dict[str, Any], replay: dict[str, Any]) -> dict[str, Any]:
    output = context["output"]
    training = _load_json(output / "training_result.json", "V77 training result")
    verification = _load_json(output / "verification.json", "V77 verification")
    access = _load_json(output / "data_access.json", "V77 data access")
    checks = {
        "all_input_phase_contract_source_and_data_hashes_match": _load_json(
            output / "preflight.json", "V77 preflight"
        )["audit"]["passed"],
        "clean_committed_git_and_registered_storage_policy": context[
            "source_receipt"
        ]["git_clean"]
        and _load_json(
            output / "backup_policy_receipt.json", "V77 backup policy"
        )["verified"],
        "mps_float32_finite_and_fallback_disabled": _load_json(
            output / "smoke.json", "V77 smoke"
        )["audit"]["passed"],
        ...
        "one_train_only_feature_scaler_per_fold_shared_across_seeds": len(
            _load_json(output / "scaler_manifest.json", "V77 scaler manifest")[
                "folds"
            ]
        )
        == 3,
    ...
    audit = {
        "schema_version": "v77-training-audit/v1",
        "passed": passed,
        "checks": checks,
    }
    audit["audit_sha256"] = canonical_sha256(audit)
    result = {
        "schema_version": "v77-persistent-duration-training-result/v1",
        "family_id": context["contract"]["family_id"],
        "decision": decision,
    ...
    _atomic_text(
        ...
        "\n".join(
            [
                "# V77 Persistent Multi-Horizon Duration Training",
                "",
                f"Decision: **{decision}**",
                "",
                ...
                "A pass authorizes only a separate outcome-blind V78 evaluation preparation.",
                "",
            ]
        ),
    )
    ...
    manifest = {
        "schema_version": "v77-artifact-manifest/v1",
    ...
    completion = {
        "schema_version": "v77-completion-receipt/v1",
        "family_id": context["contract"]["family_id"],
        "decision": decision,
        "audit_passed": passed,
        "checkpoint_count": 9,
    ...
    if actual != sorted(packet_files):
        raise V77TrainingError(f"V77 artifact packet file-set drift: {actual}")
    if not passed:
        raise V77TrainingError("V77 terminal audit failed")
    return result


def run_persistent_duration_training(
    config: dict[str, Any], *, mode: Mode
) -> dict[str, Any]:
    if mode not in MODES:
        raise ValueError(f"Unsupported V77 training mode: {mode}")
